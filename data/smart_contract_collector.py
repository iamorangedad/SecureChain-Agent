...
class SmartContractCollector:
    def __init__(
        self, github_token: str = None, output_dir: str = "collected_contracts"
    ):
    ...
    def search_github_repos(
        self, query: str, language: str, max_results: int = 100
    ) -> List[Dict]:
        """搜索GitHub仓库"""
        repos = []
        page = 1
            ...
            for result in code_results:
                try:
                    ...
                    if content:
                        ...
                        # 保存到文件
                        filename = (
                            f"{result['repository']['name']}_{result['name']}.sol"
                        )
                    ...
                    if metadata_file.exists():
                        with open(metadata_file, "r", encoding="utf-8") as mf:
                            ...
                            for i, contract in enumerate(metadata[:10]):
                                f.write(
                                    f"{i+1}. [{contract['name']}]({contract['url']}) - {contract['repository']}\n"
                                )
                            if len(metadata) > 10:
...
def main():
    ...
    parser.add_argument(
        "--language",
        choices=["solidity", "rust", "vyper", "all"],
        default="all",
        help="要收集的语言类型",
    )
